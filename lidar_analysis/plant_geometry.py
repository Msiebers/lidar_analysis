...
def _infer_crown_center(
    ...
    inferred_x = float(np.median(x_m[crown_indices]))
    inferred_z = float(np.median(z_m[crown_indices]))
    center_x = float(cfg["center_x_m"]) if cfg.get("center_x_m") is not None else inferred_x

    if cfg.get("center_z_m") is not None:
        center_z = float(cfg["center_z_m"])
        z_source = "configured_z"
    elif (
        bool(cfg.get("use_target_center_z", True))
        and context.get("target_center_z_m") is not None
        and np.isfinite(float(context["target_center_z_m"]))
    ):
        center_z = float(context["target_center_z_m"])
        z_source = "marker_center_z"
    else:
        center_z = inferred_z
        z_source = "high_point_density_z"
...
def compute_plant_geometry_traits(
    ...
    *,
    ...
    center_x_m, center_z_m, center_source = _infer_crown_center(
        x_m, z_m, height_m, cfg, context, footprint_grid_m
    )
    diagnostics.update({
