...
def test_topology_trait_reads_count_from_dict_result(monkeypatch):
    def _fake_topology_stand_count(_df, min_persistence=0.35):
        return {"count": 12.5, "points": [(1.0, 0.2, 0.8)]}

    monkeypatch.setattr("lidar_analysis.pointcloud_ops.topology_stand_count", _fake_topology_stand_count)
    df = pd.DataFrame({"X":[0.0,20.0],"Y":[100.0,100.0],"Z":[0.0,200.0],"RSSI":[1.0,2.0]})
    out = apply_pointcloud_ops(_target(df), [{"op":"topology_trait"}])
    assert out.traits["topo_count"] == 12.5
