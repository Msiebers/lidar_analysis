...
def test_compute_fad_traits_can_skip_layer_columns():
    ...
    assert traits["fad_n_supported_layers"] == 2
    assert traits["fad_profile_support_fraction"] == pytest.approx(1.0)
...
def test_layer_diagnostics_expose_unsupported_profile_without_changing_legacy_integral():
    traits = compute_fad_traits(
        origins_m=np.array([[0.0, 0.5, 0.0]]),
        directions_m=np.array([[0.0, 0.0, 1.0]]),
        ranges_m=np.array([2.0]),
        raw_hit_mask=np.array([True]),
        box=Box3D(-1.0, 1.0, 0.0, 2.0, 1.0, 3.0),
        layer_thickness_m=1.0,
        include_layer_columns=True,
    )

    assert traits["fad_lai_from_layers"] == pytest.approx(2.0)
    assert traits["fad_n_supported_layers"] == 1
    assert traits["fad_profile_support_fraction"] == pytest.approx(0.5)
    assert traits["fad_layer_100_200_n_intersecting"] == 0
    assert traits["fad_layer_100_200_n_observed"] == 0
    assert math.isnan(traits["fad_layer_100_200_m2_m3"])
